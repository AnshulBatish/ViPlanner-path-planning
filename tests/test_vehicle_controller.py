...
def test_pure_pursuit_tracks_straight_trajectory():
    ...
    command = controller.compute(plan, VehicleState(speed_mps=2.0, forward_speed_mps=2.0))
...
def test_invalid_viplanner_plan_triggers_brake_failsafe():
    ...
    command = controller.compute(
        PathPlan(planner_mode="viplanner", valid=False),
        VehicleState(speed_mps=4.0, forward_speed_mps=4.0),
    )
...
def test_longitudinal_control_brakes_when_vehicle_is_reversing():
    controller = VehicleController(PipelineConfig(max_brake=0.8))
    plan = PathPlan(
        planner_mode="viplanner",
        valid=True,
        trajectory_vehicle=np.array([[1.0, 0.0, 0.0], [4.0, 0.0, 0.0]], dtype=np.float32),
    )

    command = controller.compute(plan, VehicleState(speed_mps=2.0, forward_speed_mps=-1.5))
